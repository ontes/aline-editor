...
pub const Angle = struct {
    ...
    pub fn beginGrab(prop: *Angle) void {
        prop.grab = .{ .old_val = prop.val, .prev_mouse_pos = input.mousePos()[1] / input.windowSize()[1] };
    }
    ...
    pub fn onEvent(prop: *Angle, event: platform.Event) !void {
        if (prop.grab) |*grab| {
            switch (event) {
                .mouse_move => {
                    const mouse_pos = input.mousePos()[1] / input.windowSize()[1];
                    const multiplier: f32 = if (input.isShiftPressed()) 0.1 else 1;
